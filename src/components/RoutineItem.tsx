...
const RoutineItem: React.FC<RoutineItemProps> = ({ 
  ...
  const categoryColors: Record<string, string> = {
    ...
    lunch: 'bg-red-100 border-red-300',
  };
  ...
  const categoryClass = categoryColors[item.category.toLowerCase()] || 'bg-gray-100 border-gray-300';
  ...
  return (
    ...
    >
      <div className="flex items-start space-x-2">
        <button 
          onClick={isCompleted ? handleToggleCheck : handleComplete}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-1">
            <h3 className={`font-medium text-sm truncate ${isCompleted ? 'text-gray-700' : 'text-gray-900'}`}>
              {item.title}
            </h3>
            <div className="flex-shrink-0 flex items-center space-x-1">
              ...
              <div className="flex items-center space-x-0.5 bg-yellow-100 px-1.5 py-0.5 rounded border border-yellow-300">
                ...
                <span>{item.coinValue * completionCount === Infinity ? 'Infinite' : item.coinValue * completionCount}</span>
              </div>
            </div>
          </div>
